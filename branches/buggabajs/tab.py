...
class Tab:
	...
	def __init__(self, midifile):
		...
		self.ticksPerQuarter = f.ticksPerQuarter
		self.string=[[],[],[],[],[],[],]
		#for t in xrange(1,7):
		for track in f.tracks:
			...
			for event in track.events:
				...
				if not startevent:
					...
					if (event.type=="NOTE_OFF" and startevent.pitch==event.pitch)\ #when the corresponding NOTe_OFF event shows up, make the note
